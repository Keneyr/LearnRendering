...
public:
    MeshTriangle(const std::string& filename)
    {
        ...
        for (auto& tri : triangles)
            ptrs.push_back(&tri);
        //对该3d物体的所有三角面片求bvh，感觉没啥用，所以先删掉试试看,发现是有用的，fuck
        //bvh = new BVHAccel(ptrs); //如果用SAH，这里需要改吗
        bvh = new BVHAccel(ptrs,1,BVHAccel::SplitMethod::SAH); //fuck，改了报错
    ...
    //这个函数好奇怪啊····为什么搞了两个重名函数，都不知道啥时候调用谁
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (fabs(det) < EPSILON)
        return inter;
    double det_inv = 1. / det; //1/dot(s1,e1)
    ...
    //inter.coords = u*v0 + v*v1 + (1-u-v)*v2; //光线和三角形交点的坐标
