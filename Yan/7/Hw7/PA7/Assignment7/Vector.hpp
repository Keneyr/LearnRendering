...
public:
    ...
    //判断相等牵扯到精度问题，这个函数需要改动，因为float判断绝对相等非常困难，只能近似相等
    bool Equal(const Vector3f &p1)
    {
        //return (this->x == p1.x && this->y == p1.y && this->z == p1.z);
        return (std::abs(this->x - p1.x) < DELT && std::abs(this->y - p1.y) < DELT && std::abs(this->z - p1.z) < DELT);
    }
};
