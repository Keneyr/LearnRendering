...
public:
        ...
        : v0(_v0), v1(_v1), v2(_v2), m(_m)
    {
        ...
        area = crossProduct(e1, e2).norm(); //三角形的面积=|AB x AC| / 2
    }
...
public:
    MeshTriangle(const std::string& filename, Material *mt = new Material())
    {
        ...
        for (auto& tri : triangles){
            ptrs.push_back(&tri);
            area += tri.area; //该模型的area = 所有三角面片的area之和
        }
    ...
    void Sample(Intersection &pos, float &pdf){
        bvh->Sample(pos, pdf);
        pos.emit = m->getEmission(); //pos.emit就是公式中的li，表示直接光源照射过来的光线
    }
    float getArea(){
        return area;
    }
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    inter.normal = normal; //normal已经normalize()过了，所以不需要再次normalize()了
    inter.m = m;
    inter.obj = this;
